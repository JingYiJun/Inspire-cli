...
class InspireAPI:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries:
                    ...
                    raise InspireAPIError(
                        f"Connection error after {self.config.max_retries} retries: {str(e)}"
                    )
                ...
                if not self.config.verify_ssl:
                    logger.warning(
                        "SSL error detected, but SSL verification is disabled. This may be normal with corporate proxies."
                    )
                if attempt < self.config.max_retries:
                    ...
                    raise InspireAPIError(
                        f"SSL error after {self.config.max_retries} retries: {error_msg}"
                    )
    ...
    def _make_request(
        ...
        try:
            ...
            raise InspireAPIError(
                f"HTTP error while requesting {endpoint}: {error_summary}"
            ) from http_err
        except json.JSONDecodeError:
            body_preview = (
                response.text[: self.ERROR_BODY_PREVIEW_LIMIT] + "..."
    ...
    def create_training_job_smart(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info("🎯 Smart resource matching:")
