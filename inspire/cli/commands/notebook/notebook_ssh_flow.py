...
def _resolve_bridge_identity(
    *,
    ...
    if existing_name and _normalize_identifier(existing_name) == _normalize_identifier(
        preferred_name
    ):
...
def run_notebook_ssh(
    ...
    *,
    ...
    pubkey: Optional[str],
    aliases: tuple[str, ...] = (),
    save_as: Optional[str | tuple[str, ...]] = None,
    port: int,
    ssh_port: int,
    ...
    if isinstance(save_as, tuple):
        merged_aliases.extend(save_as)
